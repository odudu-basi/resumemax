...
export default function OnboardingStep2a() {
  const router = useRouter();
  const { onboardingData, updateOnboardingData } = useOnboarding();
  const [file, setFile] = useState<File | null>(null);
  const [isLoading, setIsLoading] = useState(false);

  // Load existing file from context on mount
  useEffect(() => {
    if (onboardingData.resumeFile) {
      setFile(onboardingData.resumeFile);
    }
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      updateOnboardingData('resumeFile', selectedFile);
    }
  };
  ...
  const handleContinue = async () => {
    if (file) {
      setIsLoading(true);
      // Small delay for UX, then navigate to step 2b where parsing will happen
      await new Promise(resolve => setTimeout(resolve, 300));
      // Navigate to step 2b (resume parsing and presentation)
            ...
            >
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-lg text-gray-600">
                  Let's start by uploading your current resume so I can help you optimize it
                </p>
              </div>
