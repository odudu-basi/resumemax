...
WORK EXPERIENCE:
${userProfile.workExperience.map((exp, i) => `${i + 1}. ${exp.position} at ${exp.company}${exp.location ? ` (${exp.location})` : ''} (${exp.duration})
   ${exp.description}`).join('\n')}

EDUCATION:
${userProfile.education.map((edu, i) => `${i + 1}. ${edu.degree} from ${edu.school} (${edu.dateRange})`).join('\n')}
